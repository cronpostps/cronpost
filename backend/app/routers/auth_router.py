...
# TURNSTILE_VERIFY_RATE_LIMIT không còn áp dụng trực tiếp ở đây vì logic đã chuyển đi
...
def generate_random_password(l: int=6) -> str: return ''.join(secrets.choice(string.ascii_letters+string.digits) for _ in range(l))

async def send_payload_to_n8n_task(payload: dict):
    if not N8N_EMAIL_WEBHOOK_URL: logger.error(f"N8N URL not set."); return
    try:
        async with httpx.AsyncClient() as client: r=await client.post(N8N_EMAIL_WEBHOOK_URL,json=payload,timeout=10.0); r.raise_for_status()
        logger.info(f"Payload for {payload.get('email_type')} sent to n8n for {payload.get('receiver_email')}.")
    except httpx.HTTPStatusError as e_http: logger.error(f"HTTP error sending to n8n: {e_http.response.status_code} - {e_http.response.text}", exc_info=False)
    except httpx.RequestError as e_req: logger.error(f"Request error sending to n8n: {e_req}", exc_info=False)
    except Exception as e: logger.error(f"Unexpected error sending to n8n: {e}", exc_info=True)
...
async def create_and_dispatch_confirmation_email_payload(db: AsyncSession,user:User,bg:BackgroundTasks,is_resend:bool=False):
    ...
    link=f"{FRONTEND_BASE_URL}/api/auth/confirm-email?token={token}"; payload={"receiver_email":user.email,"user_name":user.user_name or user.email.split('@')[0],"confirmation_link":link,"token_lifespan_hours":EMAIL_CONFIRMATION_TOKEN_LIFESPAN_HOURS,"email_type":"resend_confirmation" if is_resend else "signup_confirmation"}
    bg.add_task(send_payload_to_n8n_task,payload); logger.info(f"Dispatched '{payload['email_type']}' for {user.email}.")

async def dispatch_send_google_welcome_email(email:str,name:Optional[str],pw:str,bg:BackgroundTasks):
    payload={"receiver_email":email,"user_name":name or email.split('@')[0],"random_password":pw,"email_type":"welcome_google"}
    bg.add_task(send_payload_to_n8n_task,payload); logger.info(f"Dispatched 'welcome_google' for {email}.")

# Hàm verify_turnstile_captcha đã được di chuyển sang services/captcha_service.py
# và được import ở trên. Do đó, định nghĩa này bị xóa khỏi auth_router.py.
# @limiter.limit(TURNSTILE_VERIFY_RATE_LIMIT)
# async def verify_turnstile_captcha(request: FastAPIRequest, token: str, ip: Optional[str] = None) -> bool:
#    ... (Logic cũ đã bị xóa) ...
...
async def signup_user_endpoint(ud:UserCreateRequest,request:FastAPIRequest,bg:BackgroundTasks,db:AsyncSession=Depends(get_db_session)):
    ...
    if not await verify_turnstile_captcha( # Giả sử hàm này được import từ services.captcha_service
        token=ud.captchaToken,
    ...
    if user_for_response: 
        ...
        new_user_obj=User(
            ...
            user_name=email_prefix, # Gán user_name ở đây
            provider='email' 
            # is_confirmed_by_email và account_status sẽ lấy giá trị default từ model
        )
        db.add(new_user_obj);
        try: 
            await db.flush() 
            ...
            if user_check_after_integrity_error: 
                 ...
                 if not user_check_after_integrity_error.user_name:
                     user_check_after_integrity_error.user_name = ud.email.split('@')[0]
...
async def google_oauth_callback(
    ...
    try:
        token_response = await oauth_client.fetch_token(
            url=token_endpoint_url, code=code,
            redirect_uri=GOOGLE_REDIRECT_URI_FROM_ENV,
            code_verifier=code_verifier
        )
        id_token_str = token_response.get('id_token')
        if not id_token_str:
            logger.error("Missing 'id_token' string from Google.")
            raise Exception("Missing id_token string")

        async with httpx.AsyncClient() as http_client:
            jwks_response = await http_client.get(jwks_uri, timeout=5.0)
    ...
    except JoseJWTError as e_jwt:
        logger.error(f"Error decoding/validating Google ID token: {e_jwt}", exc_info=True)
        return RedirectResponse(url=f"{FRONTEND_BASE_URL}/signin.html?status=google_oauth_token_invalid")
    except Exception as e:
        logger.error(f"Error during Google token exchange or ID token processing: {e}", exc_info=True)
        return RedirectResponse(url=f"{FRONTEND_BASE_URL}/signin.html?status=google_oauth_token_error")
    ...
    final_redirect_url = f"{FRONTEND_BASE_URL}/dashboard.html?access_token={access_token}&status={status_param}"
    logger.info(f"Google OAuth OK for {user.email}. Redirecting."); return RedirectResponse(url=final_redirect_url)