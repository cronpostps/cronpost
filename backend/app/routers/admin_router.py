...
# Version 1.6.1 (Corrected Reset PIN logic)
...
from ..db.models import SystemSetting, User, UserMembershipTypeEnum, UserAccountStatusEnum, Message, FmSchedule, SimpleCronMessage
from ..dependencies import get_current_admin_user
from ..services.email_service import send_email_async

# Correctly import verify_password from auth_router
...
class SystemSettingResponse(BaseModel):
    setting_key: str; setting_value: Optional[str] = None; description: Optional[str] = None
    class Config: from_attributes = True

class SettingUpdateRequest(BaseModel):
    value: str; admin_pin: constr(pattern=r"^\d{4}$")

class PinVerifyRequest(BaseModel):
    admin_pin: constr(pattern=r"^\d{4}$")

class UserAdminView(BaseModel):
    id: uuid.UUID; email: EmailStr; user_name: Optional[str] = None; membership_type: UserMembershipTypeEnum
    account_status: UserAccountStatusEnum; last_activity_at: Optional[datetime] = None; created_at: datetime
    class Config: from_attributes = True

class UserListResponse(BaseModel):
    total_count: int; users: List[UserAdminView]
...
async def verify_admin_pin(pin_data: PinVerifyRequest, current_admin: User = Depends(get_current_admin_user)):
    if not current_admin.pin_code or not verify_password(pin_data.admin_pin, current_admin.pin_code):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Incorrect Admin PIN.")
...
async def update_system_setting(setting_key: str, update_data: SettingUpdateRequest, current_admin: User = Depends(get_current_admin_user), db: AsyncSession = Depends(get_db_session)):
    if not current_admin.pin_code or not verify_password(update_data.admin_pin, current_admin.pin_code):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Incorrect Admin PIN.")
    ...
    setting.setting_value = update_data.value; await db.commit()
...
async def get_users_list(db: AsyncSession = Depends(get_db_session), skip: int = 0, limit: int = 10, search: Optional[str] = None, sort_by: Optional[Literal['membership_type', 'last_activity_at', 'created_at']] = Query('created_at'), sort_dir: Optional[Literal['asc', 'desc']] = Query('desc')):
...
async def upgrade_user_to_premium(user_id: uuid.UUID, action_data: UserActionRequest, current_admin: User = Depends(get_current_admin_user), db: AsyncSession = Depends(get_db_session)):
    if not current_admin.pin_code or not verify_password(action_data.admin_pin, current_admin.pin_code):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Incorrect Admin PIN.")
    ...
    target_user.membership_type = UserMembershipTypeEnum.premium; target_user.membership_expires_at = None
    await db.commit(); await db.refresh(target_user)
...
async def downgrade_user_to_free(user_id: uuid.UUID, action_data: UserActionRequest, current_admin: User = Depends(get_current_admin_user), db: AsyncSession = Depends(get_db_session)):
    if not current_admin.pin_code or not verify_password(action_data.admin_pin, current_admin.pin_code):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Incorrect Admin PIN.")
    target_user = await db.get(User, user_id)
    if not target_user: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found.")
...
async def delete_user(user_id: uuid.UUID, action_data: UserActionRequest, current_admin: User = Depends(get_current_admin_user), db: AsyncSession = Depends(get_db_session)):
    if not current_admin.pin_code or not verify_password(action_data.admin_pin, current_admin.pin_code):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Incorrect Admin PIN.")
    target_user = await db.get(User, user_id)
    if not target_user: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found.")
    ...
    await db.delete(target_user); await db.commit()
...
async def reset_user_pin(
    ...
    # CORRECTED LOGIC: Clear all PIN-related info and disable dependent settings
    ...
    # Send notification email using the new template
    ...
    await db.commit()
    logger.info(f"Admin '{current_admin.email}' reset PIN for user '{target_user.email}'.")
    return MessageResponse(message=f"PIN for user {target_user.email} has been reset. A notification email has been sent.")

# --- DEBUG ENDPOINT ---
@router.get("/debug-ip", summary="Debug client IP and headers")
async def debug_ip_and_headers(request: Request):
    headers = dict(request.headers)
    debug_info = {"message": "Debug information.", "request_client_host": request.client.host if request.client else "N/A", "headers": headers}
    logger.info(f"DEBUG IP ENDPOINT ACCESSED. INFO: {debug_info}")
